...
function D3SI(container, data, parameters) {
    ...
    this.minSeriesValue = undefined
    this.maxSeriesValue = undefined
    this.xScale = undefined
    this.yScale = undefined
    this.colourScale = undefined
...
D3SI.prototype.groupDataByIndex = function (indexCol) {
    ...
    newdata = this.data.map(function(row) { // take each row
    ...
    // Remember the min and max value across the series, so we can set the scales correctly later
    this.minSeriesValue = d3.min(newdata, function(c) { return d3.min(c.values, function(d) { return d.value }) })
    this.maxSeriesValue = d3.max(newdata, function(c) { return d3.max(c.values, function(d) { return d.value }) })
    return newdata
}
...
D3SI.prototype.groupDataBySeries = function(indexCol) {
    ...
    data = this.data
    series = data.columns.slice(1)
    newdata = series.map(function(s) { // take each series column
        return {
            series: s,
...
D3SI.prototype.xScaleBand = function(colName) {
    list = this.data.map(function(d){return d[colName]})
    ...
    this.xScaleOffset = this.xScale.bandwidth()/2
    return this.xScale
}
...
D3SI.prototype.colourScaleOrdinal = function(colName, colours) {
    list = this.data.map(function(d){return d[colName]})
    this.colourScale =  d3.scaleOrdinal(colours)
        .domain(list) 
...
D3SI.prototype.colourScaleLinear = function(colName, minColour, maxColour) {
    min = getMinValue(this.data, colName)  
    max = getMaxValue(this.data, colName) 
...
D3SI.prototype.colourMap = function(column) {
    chart = this
    return function(d) { return chart.colourScale(d[column]) } 
}
...
D3SI.prototype.drawAxisXTop = function(text, ticks) {
    const ticks1 = ticks || this.data.length        // default ticks to length of data

    // Define the axis
    this.xAxis = d3.axisTop()                       // draw the axis at the top
        .scale(this.xScale)                         // use the x scaler, which must be defined first
        .ticks(ticks1)                              // set the number of ticks

    // Draw the axis
    this.svg.append("g")                                            // add a group element
        .attr("class", "xaxis")
        .attr("transform", "translate(0," + this.drawingTop + ")")  // move up to the top of the drawing area
        .call(this.xAxis)                                           // draw the axis elements
        
    // Draw the axis label
    posx = this.drawingCentreX
    posy = this.headerTop 
...
// The axis line is drawn at the border of the drawing area and padding area
// The label is draw at the outer border of the padding area
D3SI.prototype.drawAxisXBottom = function(text, ticks) {
    const ticks1 = ticks || this.data.length        // default ticks to length of data

    // Define the axis
    this.xAxis = d3.axisBottom()                    // draw the axis at the bottom
        .scale(this.xScale)                         // use the x scaler, which must be defined first
        .ticks(ticks1)                              // set the number of ticks
    ...
    this.svg.append("g")                                                // add a group element
        ...
        .call(this.xAxis)                                               // draw the axis
    
    // Draw the axis label
    posx = this.drawingCentreX
    posy = this.height - this.headerBottom
    this.svg.append("text")       
      .text(text)
      .attr("class", "xlabel")     
      .attr("y", posy)
...
// Update the x-axis 
D3SI.prototype.updateXAxis = function(ticks) {
...
// The axis line is drawn at the border of the drawing area and padding area
// The label is draw at the outer border of the padding area
D3SI.prototype.drawAxisYLeft = function(text, ticks) {
    const ticks1 = ticks || this.data.length
...
// The axis line is drawn at the border of the drawing area and padding area
// The label is draw at the outer border of the padding area
D3SI.prototype.drawAxisYRight = function(text, ticks) {
    const ticks1 = ticks || this.data.length
...
// Update the y-axis 
D3SI.prototype.updateYAxis = function(ticks) {
    const ticks1 = ticks || this.data.length

    // Redefine the axis
    this.yAxis
        .scale(this.yScale)                 // use the y scaler, which must be defined first
        .ticks(ticks1)                      // set the number of ticks
...
D3SI.prototype.getPieArcGenerator = function(innerRadiusPercent) {
    ...
    var innerRadius = innerRadiusPercent * this.outerRadius 
    return d3.arc()
...
D3SI.prototype.getPartitionLabelGenerator = function() {
    padLeft = this.paddingLeft+this.headerLeft
    padTop = this.paddingTop+this.headerTop
    return function(d) {
...
D3SI.prototype.bars = function(bindings, xCol, yCol) {
    xScale = this.xScale
    yScale = this.yScale    
    var miny = 0 //or may want to use d3.min to find min y
...
D3SI.prototype.hbars = function(bindings, xCol, yCol) {
    xScale = this.xScale
    yScale = this.yScale    
    var chart = this
    var minx = 0 //or may want to use d3.min to find min x
...
D3SI.prototype.seriesBars = function(bindings) {
    xScale = this.xSeriesScale
    yScale = this.yScale    
    var miny = 0 //this.minSeriesValue
...
D3SI.prototype.getLineGenerator = function(xCol, yCol) {
    xScale = this.xScale
    yScale = this.yScale    
    return d3.line()
...
D3SI.prototype.line = function(bindings, linepoints) {
    return bindings
        //.append('path')
        .attr("class", "line")
        .attr("d", linepoints)
...
D3SI.prototype.seriesLines = function(bindings, linepoints) {
...
D3SI.prototype.fillxy = function(bindings, xCol, yCol) {
    xScale = this.xScale
    yScale = this.yScale    
    return this.append(bindings, "rect")
        .attr("x",        function(d) { return xScale(d[xCol]) })
...
D3SI.prototype.diamondsxy = function(bindings, xCol, yCol, sizeCol) {
    xScale = this.xScale
    yScale = this.yScale    
    xOffset = this.xScaleOffset
    xScaleOffset1 = function(d) { return xScale(d[xCol]) + xOffset }
    yScaleOffset1 = function(d) { return yScale(d[yCol]) + d[sizeCol]/2 }
    return this.append(bindings, "rect")
...
D3SI.prototype.circlesxy = function(bindings, xCol, yCol, sizeCol) {
    xScale = this.xScale
    yScale = this.yScale    
    xOffset = this.xScaleOffset //xScale.bandwidth()/2
...
D3SI.prototype.circlesx = function(bindings, xCol, y, sizeCol) {
    xScale = this.xScale 
    xOffset = this.xScaleOffset
    return this.append(bindings, "circle")
        .attr("cx",  function (d) { return xScale(d[xCol]) + xOffset })
...
function buildMulticharts(chartFn, chartId, data, chartParams, chartCol) {
    ...
    // Get unique years
    chartsData = getUniqueColValues(data, chartCol) 

    // Create DIVs, one for each multichart
    var containers = chartEl
        .selectAll('div')
        .data(chartsData)
        .enter()
        .append("div")
...
function drawSimpleLegend(legendId, data, parameters={}) {
    ...
    legendItems = g.selectAll('g')
        .data(data)
        .enter()
        .append('g')

    // Add the title
    titleSelection = this.svg.append("text")       
        .text(title)
    ...
    titleHeight = 5 + titleSelection.node().getBBox().height
    yoffset = titleHeight + itemHeight/2-boxHeight/2
